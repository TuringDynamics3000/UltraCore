...
DATABASE_URL=postgresql://postgres:password@localhost:5432/ultracore_operations
...
````bash
# Create the database (using PostgreSQL command line)
psql -U postgres -c "CREATE DATABASE ultracore_operations;"
...
- **AWS**: Deploy on EC2, ECS, or EKS with RDS PostgreSQL or Aurora PostgreSQL
...
psql -U postgres -d ultracore_operations -c "\dt"

# Check if database exists
psql -U postgres -l | grep ultracore_operations
```
